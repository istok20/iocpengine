...
unit DnTcpChannel;
interface
uses JwaWinsock2, contnrs, Windows, Classes, SysUtils,
...
begin
  ...
  FRunGuard := TDnMutex.Create;
  FReactor := Reactor;
  FSocket := JwaWinsock2.WSASocket(AF_INET, SOCK_STREAM, IPPROTO_TCP, Nil, 0, WSA_FLAG_OVERLAPPED);
...
begin
  ...
  FRemoteAddr.sin_addr.S_addr := JwaWinsock2.inet_addr(PAnsiChar(RemoteIP));
...
begin
  ...
  try
    if FSocket <> INVALID_SOCKET then
    begin
      JwaWinsock2.shutdown(FSocket, SD_SEND);
...
begin
  Result := FSocket = JwaWinsock2.INVALID_SOCKET;
end;
...
begin
  Temp := Value;
  JwaWinsock2.setsockopt(FSocket, IPPROTO_TCP, TCP_NODELAY, PAnsiChar(@Temp), SizeOf(Temp));
end;
